...
export default function App() {
  ...
  const aliveTargets = game.players.filter((player) => player.status === "alive" && (game.phase === "guard_action" || player.seatId !== human.seatId) && !human.wolfMates?.includes(player.seatId));
...
function Welcome({ loading, error, onStart }: { loading: boolean; error: string; onStart: () => void }) {
  return <main className="welcome-shell"><div className="welcome-copy"><p className="eyebrow">A PRIVATE TABLE FOR TWELVE</p><div className="hero-mark">✦</div><h1>每一句话<br /><em>都有立场。</em></h1><p className="welcome-description">十二个座位，一张暗桌。和十一个有记忆、会判断、也会撒谎的 AI 玩家，打一局完整的狼人杀。</p><button className="primary-button enter-button" disabled={loading} onClick={onStart}>{loading ? "正在点亮牌桌…" : "进入夜色"}<span>→</span></button>{error && <p className="error-copy">{error}</p>}</div><div className="welcome-side"><span>01</span><div className="side-line" /><p>四狼阵容<br />守卫加入夜行<br />每局事件存档</p></div></main>;
}
...
function ActionPanel({ game, canAct, target, setTarget, speech, setSpeech, witchChoice, setWitchChoice, aliveTargets, sendCommand }: {
  ...
  if (game.phase === "guard_action") return <><h3>守住一个人。</h3><p className="action-hint">今晚的守护只会抵消狼刀，可以守护自己。</p><TargetSelect value={target} onChange={setTarget} targets={aliveTargets} placeholder="选择守护目标" /><button className="primary-button" disabled={!target} onClick={() => void sendCommand("guard.protect", { targetSeatId: target })}>确认守护 <span>→</span></button></>;
...
function TimelineEvent({ event, game }: { event: GameEvent; game: PublicState }) {
  ...
  if (event.type === "guard.result") return <div className="timeline-item private-item"><span className="event-glyph">◇</span><p>守护目标：<strong>{playerName(game, String(event.payload.targetSeatId))}</strong>。</p></div>;
